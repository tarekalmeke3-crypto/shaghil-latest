        ...
        if (error) {
            ...
            if (taskData?.company_id) {
                await supabase.from('notifications').insert({
                    user_id: taskData.company_id,
                    message: `متقدم جديد على مهمتك "${task.title}"`,
                    link: `/company/tasks/${task.id}/applicants`,
                });
            }
                ...
                <div className="bg-deep-charcoal rounded-2xl p-8 border border-white/5">
                    <div className="flex items-start justify-between">
                        ...
                        <span className={`px-3 py-1 text-sm rounded-full ${task.status === 'ACTIVE' ? 'bg-green-500/10 text-green-400 border border-green-500/30' : 'bg-ash-grey/10 text-ash-grey'}`}>
                            {task.status === 'ACTIVE' ? 'نشطة' : task.status === 'COMPLETED' ? 'مكتملة' : task.status}
                        </span>
                    </div>
                    ...
                    <div className="mt-10">
                        {isCompleted ? (
                            <p className="text-center text-lg font-bold text-ash-grey">
                                هذه المهمة مكتملة ولا يمكن التقديم عليها
                            </p>
                        ) : successMsg ? (
                            ...
                            <p className="text-yellow-400 text-center text-lg font-bold">
                                لقد قدّمت على هذه المهمة بالفعل
                            </p>
                        ) : (
                            <Button
                                onClick={handleApply}
